...
class hGRUCell(nn.Module):
    ...
    def __init__(self,
        ...
        if self.normalization == 'batchnorm':
            self.bn = nn.ModuleList([nn.BatchNorm2d(hidden_size) for _ in range(4)])
            for bn in self.bn:
                init.constant_(bn.weight, bn_weight)
            self.do_norm = True
        elif self.normalization == 'layernorm':
            self.bn = nn.ModuleList([nn.GroupNorm(1, hidden_size) for _ in range(4)])
            self.do_norm = True
        else:
    ...
    def forward(self, input_, previous_state, timestep=0):
        ...
        if timestep == 0 and h2_tminus1 is None:
